...
def display_data_dict(data_dict):
    for key in list(data_dict.keys())[::2]:
        ...
        # Check if there's a next key to display in the second column
...
def extract_data(text):
    ...
    st.write(text)
    info = {
        "sponsor Bank code": "alpha numeric of length 11",
        "utility code": "7 digit number",
        "Bank Account Number": "15 digit number",
        "IFSC ": "alpha numeric of length 11",
        "MICR": "9 digit number",
        "policy no": "9 digit number",
        "division code": "three digit number"
    }
 
    instruction = f"Use few entity information from {info} and extract all data from the text = {text} as json object without any note."
    #instruction = f"extract all data from the text = {text} as json object without any note."
    # Build the question and template for prompting
    question = f"{instruction}, ocr_text={text}"
    template = "Question: {question}\nAnswer: "
    prompt = PromptTemplate(template=template, input_variables=["question"])

    # Chain the prompt and HuggingFaceEndpoint
    ...
    # var = type(data_dict)
    # st.write(var)
    # Display each entity as a button or dropdown
    st.write("\n")
    #st.markdown("### Select an entity to display corresponding information:")

    st.write("### Entities Information")
